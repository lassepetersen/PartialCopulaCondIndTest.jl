...
@testset "Trimmed Spearman correlation test" begin
    n = 1000
    q = 10
    ...
    TSCT = PartialCopulaCondIndTests.TrimmedSpearmanCorrelationTest(q, U₁, U₂)

    @test PartialCopulaCondIndTests.nobs(TSCT) == n
    @test isfinite(PartialCopulaCondIndTests.pvalue(TSCT))
    @test PartialCopulaCondIndTests.pvalue(TSCT) >= 0.0
    @test PartialCopulaCondIndTests.pvalue(TSCT) <= 1.0
end
